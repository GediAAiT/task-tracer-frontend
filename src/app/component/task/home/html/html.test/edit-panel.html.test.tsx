...
describe('EditPanelSection', () => {
  it('shows the edit form inside a dialog once a task is being edited', () => {
  ...
  it('stays closed until a task is being edited', () => {
    render(<EditPanelSection panel={panel({ open: false })} />);

    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
  });

  it('discards the edit when the dialog is dismissed', async () => {
    const onOpenChange = vi.fn();
    const user = userEvent.setup();

    render(<EditPanelSection panel={panel({ onOpenChange })} />);
